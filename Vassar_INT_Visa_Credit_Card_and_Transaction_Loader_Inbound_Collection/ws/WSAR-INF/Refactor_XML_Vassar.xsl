...
<!-- Changed the substring on Credit card Account number to be on the last 6 digits instead of 4, due to employees with identical last 4 digit CC number    -->
    ...
    <xsl:template match="/Transaction">		
        ...
        <xsl:choose>
            <xsl:when test="$eight.record = false() and $nine.record = false() and $seven.record=false()">
                <Transaction>
                    <record_type>
                        <xsl:value-of select="$record.type"/>
                    </record_type>
                    
                     <xsl:if test="$record.type = '3'">
                       <Record_03>
                       <corp_num>
                    ...
                    <xsl:if test="$record.type = '4'">
                       <Record_04>
                        <acct_num>
                            <xsl:value-of select="normalize-space(substring(Field2,17,4))"/>
                        </acct_num>
                         <acct_num_token>
                            <xsl:value-of select="normalize-space(Field2)"/>
                        </acct_num_token>
...
<!--                     </xsl:if> -->
            ...
            <xsl:when test="$seven.record = true()">
                <Transaction>
                    <record_type>
                        <xsl:text>7</xsl:text>
                    </record_type>
